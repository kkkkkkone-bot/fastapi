...
func authHelper(c *gin.Context, minRole int) {
	...
	if sessionID, ok := session.Get("id").(int); ok && sessionID > 0 {
		var err error
		user, err = model.GetUserCache(sessionID)
		if err != nil {
			common.SysLog(fmt.Sprintf("authHelper GetUserCache error for user %d: %v", sessionID, err))
		...
		// Check access token when there is no valid browser session identity.
		...
		accessUser, authErr := model.ValidateAccessToken(accessToken)
		if authErr != nil {
		...
		if accessUser != nil && accessUser.Username != "" {
			if !validUserInfo(accessUser.Username, accessUser.Role) {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
			...
			user, authErr = model.GetUserCache(accessUser.Id)
			if authErr != nil {
				common.SysLog(fmt.Sprintf("authHelper GetUserCache error for access token user %d: %v", accessUser.Id, authErr))
				c.JSON(http.StatusInternalServerError, gin.H{
					"success": false,
					"message": common.TranslateMessage(c, i18n.MsgDatabaseError),
				})
