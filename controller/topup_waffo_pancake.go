...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 收到请求 path=%q client_ip=%s payload=%s", c.Request.RequestURI, c.ClientIP(), payloadFingerprint(bodyBytes)))

	event, err := service.VerifyConfiguredWaffoPancakeWebhook(string(bodyBytes), signature)
	if err != nil {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 验签失败 path=%q client_ip=%s payload=%s error=%q", c.Request.RequestURI, c.ClientIP(), payloadFingerprint(bodyBytes), err.Error()))
