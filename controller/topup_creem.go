...
func verifyCreemSignature(payload string, signature string, secret string) bool {
	if secret == "" {
		logger.LogWarn(context.Background(), fmt.Sprintf("Creem webhook secret 未配置 test_mode=%t payload=%s", setting.CreemTestMode, payloadFingerprint([]byte(payload))))
		if setting.CreemTestMode {
			logger.LogInfo(context.Background(), fmt.Sprintf("Creem webhook 验签已跳过 reason=test_mode payload=%s", payloadFingerprint([]byte(payload))))
			return true
		}
		return false
	}
...
func RequestCreemPay(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付请求已收到 user_id=%d payload=%s", c.GetInt("id"), payloadFingerprint(bodyBytes)))
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s payload=%s", c.Request.RequestURI, c.ClientIP(), payloadFingerprint(bodyBytes)))
	if signature == "" {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 缺少签名 path=%q client_ip=%s payload=%s", c.Request.RequestURI, c.ClientIP(), payloadFingerprint(bodyBytes)))
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s payload=%s", c.Request.RequestURI, c.ClientIP(), payloadFingerprint(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s error=%q payload=%s", c.Request.RequestURI, c.ClientIP(), err.Error(), payloadFingerprint(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	logger.LogInfo(ctx, fmt.Sprintf("Creem API 响应已收到 trade_no=%s status_code=%d payload=%s", referenceId, resp.StatusCode, payloadFingerprint(body)))
