...
func EpayNotify(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 收到请求 path=%q client_ip=%s method=%s payload=%s", c.Request.RequestURI, c.ClientIP(), c.Request.Method, payloadFingerprint([]byte(common.GetJsonString(params)))))
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		if err := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值完成失败 trade_no=%s callback_type=%s client_ip=%s error=%q", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP(), err.Error()))
			_, _ = c.Writer.Write([]byte("fail"))
			return
		}
	} else {
